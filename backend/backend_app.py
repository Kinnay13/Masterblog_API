"""Flask-based REST API for a blogging platform."""
...
def get_posts():
    ...
    if sort_by:
        ...
        if sort_by not in ['title', 'content']:
            return jsonify({
                "error": "Invalid sort field. Allowed: 'title' or 'content'"
            }), 400

        # Validate direction parameter
        if direction not in ['asc', 'desc']:
            return jsonify({
                "error": "Invalid sort direction. Allowed: 'asc' or 'desc'"
            }), 400

        # Sort posts by the specified field
        posts.sort(key=lambda post: post[sort_by].lower(),
